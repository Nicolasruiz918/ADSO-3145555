...
-- ============================================================
-- AIRLINE RESERVATION & OPERATIONS SCHEMA  v3.0
-- Basado en v2.0 de equipo. Correcciones v3:
--   FIX-V3-01  Orden de triggers corregido (tabla primero, trigger después)
--   FIX-V3-02  passenger_type como tabla lookup
--   FIX-V3-03  baggage_type y baggage_status como tablas lookup
--   FIX-V3-04  seat_assignment_source como tabla lookup
--   FIX-V3-05  boarding_validation_result como tabla lookup
--   FIX-V3-06  payment_transaction_type como tabla lookup
--   FIX-V3-07  CHECK birth_date rango válido en person
--   FIX-V3-08  address con soft-delete (is_active)
--   FIX-V3-09  fare con columna generada is_current
--   FIX-V3-10  Trigger EARN sin ticket_id detectado
--   FIX-V3-11  exchange_rate_type como tabla lookup
--   FIX-V3-12  Historial de estados (reservation, ticket, payment)
--   FIX-V3-13  airline con address_id
--   FIX-V3-14  maintenance_provider con contact_person_id FK a person
--   FIX-V3-15  boarding_pass con expires_at
--   FIX-V3-16  Índice en person(last_name, first_name)
--   FIX-V3-17  COMMENT ON extendidos
-- ============================================================
...
-- FIX-V3-08: is_active para soft-delete; evita borrar registros con referencias históricas
...
-- FIX-V3-13: address_id para registrar sede de la aerolínea
...
-- FIX v2 #8: gender como tabla lookup (conservado de v2)
CREATE TABLE gender (
...
-- FIX-V3-02: passenger_type como tabla lookup (era CHECK inline en reservation_passenger)
...
CREATE TABLE person (
    ...
    created_at             timestamptz NOT NULL DEFAULT now(),
    updated_at             timestamptz NOT NULL DEFAULT now(),
    -- FIX-V3-07: rango válido en birth_date; antes aceptaba '1700-01-01' o fechas futuras
    CONSTRAINT ck_person_birth_date CHECK (
        birth_date IS NULL OR birth_date BETWEEN '1900-01-01' AND current_date
    )
);

COMMENT ON COLUMN person.gender_id      IS 'FK a tabla gender. NULL indica dato no informado por el pasajero.';
COMMENT ON COLUMN person.birth_date     IS 'Fecha de nacimiento. Restringida entre 1900-01-01 y la fecha actual.';
...
-- FIX v2 #4: is_current con índice parcial único (conservado de v2)
...
-- Garantiza exactamente un tier activo por loyalty_account en todo momento
CREATE UNIQUE INDEX uq_loyalty_account_current_tier
    ON loyalty_account_tier (loyalty_account_id)
    WHERE is_current = true;

COMMENT ON TABLE  loyalty_account_tier            IS 'Historial de tiers. El índice parcial uq_loyalty_account_current_tier garantiza exactamente un tier activo (is_current=true) por cuenta.';
COMMENT ON COLUMN loyalty_account_tier.is_current IS 'Solo una fila puede ser true por loyalty_account_id. Actualizar dentro de transacción al cambiar de tier.';

-- FIX v2 #3: miles_transaction_type como lookup (conservado de v2)
...
-- ticket_id se agrega como columna pero la FK se aplica después de crear ticket
CREATE TABLE miles_transaction (
    ...
    ticket_id                 uuid,        -- FK a ticket; se aplica con ALTER TABLE más abajo
...
COMMENT ON TABLE  miles_transaction            IS 'Transacciones de millas. ticket_id es obligatorio cuando el tipo es EARN; NULL solo permitido para ADJUST (validado por trigger fn_check_miles_earn_ticket).';
COMMENT ON COLUMN miles_transaction.ticket_id  IS 'FK opcional a ticket. Debe estar presente cuando miles_transaction_type = EARN.';
...
COMMENT ON COLUMN airport.country_id IS 'FK desnormalizada a country para evitar 5 niveles de JOIN en consultas de vuelos domésticos vs internacionales. Debe mantenerse consistente con address → district → city → state_province → country.';
...
-- FIX-V3-14: contact_person_id FK a person en lugar de contact_name varchar libre
...
COMMENT ON COLUMN maintenance_provider.contact_person_id IS 'FK a person para el contacto principal del proveedor. Reemplaza contact_name varchar libre de v2.';
...
-- FIX v2 #14: maintenance_event_status como tabla lookup (conservado de v2)
...
-- FIX v2 #9: is_terminal para identificar estados finales (conservado de v2)
...
COMMENT ON COLUMN flight_status.is_terminal IS 'true cuando el estado implica que el vuelo ya no está en operación activa (LANDED, CANCELLED, DIVERTED). Permite validación cruzada con actual_departure_at / actual_arrival_at.';
...
-- FIX-V3-09: columna generada is_current para consultas de tarifas vigentes sin full-scan
CREATE TABLE fare (
...
COMMENT ON COLUMN fare.is_current IS 'Columna generada: true si la tarifa está vigente a la fecha actual. Usar con índice parcial idx_fare_current para consultas de disponibilidad.';
...
COMMENT ON TABLE reservation IS 'Entidad raíz del flujo comercial y de booking del sistema.';

-- FIX-V3-02: passenger_type_id FK a tabla lookup (antes CHECK inline con ADULT/CHILD/INFANT)
...
-- FIX v2 #3: FK a ticket ahora que la tabla existe
...
-- ============================================================
-- FIX-V3-01: TRIGGERS DE INTEGRIDAD
-- Tablas creadas primero; triggers después (orden corregido de v2)
-- ============================================================

-- FIX v2 #2: valida que el flight_segment corresponde a la aerolínea de la reserva
...
-- FIX-V3-10: valida que EARN siempre tenga ticket_id
...
BEGIN
    ...
    IF v_type_code = 'EARN' AND NEW.ticket_id IS NULL THEN
        RAISE EXCEPTION 'miles_transaction de tipo EARN requiere ticket_id; use tipo ADJUST para acreditaciones manuales sin ticket.';
    END IF;
...
CREATE TRIGGER trg_miles_earn_ticket_check
    ...
    FOR EACH ROW EXECUTE FUNCTION fn_check_miles_earn_ticket();

-- FIX-V3-04 y FIX-V3-01: tabla seat_assignment primero, trigger después
-- FIX-V3-04: assignment_source_id FK a tabla lookup (antes CHECK inline)
CREATE TABLE seat_assignment_source (
    seat_assignment_source_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    source_code               varchar(20) NOT NULL,
    source_name               varchar(80) NOT NULL,
    created_at                timestamptz NOT NULL DEFAULT now(),
    updated_at                timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_seat_assignment_source_code UNIQUE (source_code),
    CONSTRAINT uq_seat_assignment_source_name UNIQUE (source_name)
);
...
CREATE TABLE seat_assignment (
    ...
    -- FIX-V3-04: FK a lookup en lugar de CHECK inline (AUTO, MANUAL, CUSTOMER)
    seat_assignment_source_id uuid        NOT NULL REFERENCES seat_assignment_source(seat_assignment_source_id),
    assigned_at               timestamptz NOT NULL,
    created_at                timestamptz NOT NULL DEFAULT now(),
    updated_at                timestamptz NOT NULL DEFAULT now(),
...
COMMENT ON TABLE seat_assignment IS 'Asignación de asiento normalizada por ticket_segment. Trigger trg_seat_assignment_aircraft_check garantiza que el asiento pertenece al avión del segmento.';

-- FIX v2 #1: valida que el asiento pertenece al avión del segmento
...
CREATE TABLE baggage (
    ...
    -- FIX-V3-03: FKs a lookup en lugar de CHECK inline
...
-- FIX-V3-15: expires_at para invalidar boarding pass pasada la hora de salida
CREATE TABLE boarding_pass (
    ...
    CONSTRAINT ck_boarding_pass_dates    CHECK (expires_at IS NULL OR expires_at > issued_at)
);

COMMENT ON COLUMN boarding_pass.expires_at IS 'Momento a partir del cual el boarding pass no es válido. Normalmente coincide con la hora de salida del vuelo.';

-- FIX-V3-05: boarding_validation_result como tabla lookup (antes CHECK inline)
CREATE TABLE boarding_validation_result (
    boarding_validation_result_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    result_code                   varchar(20) NOT NULL,
    result_name                   varchar(80) NOT NULL,
    created_at                    timestamptz NOT NULL DEFAULT now(),
    updated_at                    timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_boarding_validation_result_code UNIQUE (result_code),
    CONSTRAINT uq_boarding_validation_result_name UNIQUE (result_name)
);
...
CREATE TABLE boarding_validation (
    ...
    -- FIX-V3-05: FK a lookup en lugar de CHECK inline (APPROVED/REJECTED/MANUAL_REVIEW)
    boarding_validation_result_id uuid        NOT NULL REFERENCES boarding_validation_result(boarding_validation_result_id),
...
-- FIX-V3-06: payment_transaction_type como tabla lookup (antes CHECK inline)
...
CREATE TABLE payment_transaction (
    ...
    -- FIX-V3-06: FK a lookup en lugar de CHECK inline (AUTH/CAPTURE/VOID/REFUND/REVERSAL)
...
-- FIX v2 #16: refund referencia la transacción de captura específica (conservado de v2)
CREATE TABLE refund (
...
COMMENT ON COLUMN refund.source_payment_transaction_id IS 'FK a la transacción CAPTURE que origina el reembolso. Nullable para reembolsos manuales sin transacción de captura identificada.';
...
-- FIX v2 #10: exchange_rate con fuente y tipo de tasa (conservado de v2)
...
-- FIX-V3-11: exchange_rate_type como tabla lookup (antes CHECK inline MID/BUY/SELL)
CREATE TABLE exchange_rate_type (
    exchange_rate_type_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    type_code             varchar(10) NOT NULL,
    type_name             varchar(60) NOT NULL,
    created_at            timestamptz NOT NULL DEFAULT now(),
    updated_at            timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_exchange_rate_type_code UNIQUE (type_code),
    CONSTRAINT uq_exchange_rate_type_name UNIQUE (type_name)
);

COMMENT ON TABLE exchange_rate_type IS 'MID = mid-market, BUY = tasa de compra, SELL = tasa de venta.';
...
CREATE TABLE exchange_rate (
    ...
    exchange_rate_type_id   uuid           NOT NULL REFERENCES exchange_rate_type(exchange_rate_type_id),
    effective_date          date           NOT NULL,
    rate_value              numeric(18, 8) NOT NULL,
    created_at              timestamptz    NOT NULL DEFAULT now(),
    updated_at              timestamptz    NOT NULL DEFAULT now(),
    CONSTRAINT uq_exchange_rate      UNIQUE (from_currency_id, to_currency_id, effective_date, exchange_rate_type_id),
...
COMMENT ON TABLE exchange_rate IS 'Tasas de cambio con tipo (MID/BUY/SELL) y fuente para billing multi-moneda auditado.';
...
-- FIX v2 #7: trazabilidad polimórfica del concepto facturado (conservado de v2)
...
COMMENT ON TABLE  invoice_line                IS 'Detalle facturable sin totales derivados (3FN). invoice_line_source_type_id + source_entity_id proveen trazabilidad al origen del concepto facturado.';
COMMENT ON COLUMN invoice_line.source_entity_id IS 'ID polimórfico del ticket, baggage u otro origen. El tipo está determinado por invoice_line_source_type_id.';

-- ============================================================
-- FIX-V3-12: HISTORIAL DE ESTADOS
-- Trazabilidad completa de transiciones para auditoría IATA
-- ============================================================

CREATE TABLE reservation_status_history (
    reservation_status_history_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    reservation_id                uuid        NOT NULL REFERENCES reservation(reservation_id),
    from_status_id                uuid        REFERENCES reservation_status(reservation_status_id),
...
COMMENT ON TABLE reservation_status_history IS 'Historial de transiciones de estado de reservas. from_status_id NULL indica el estado inicial al crear la reserva.';
...
COMMENT ON TABLE ticket_status_history IS 'Historial de transiciones de estado de tickets. Requerido por normas IATA para auditoría de emisión y cancelación.';
...
COMMENT ON TABLE payment_status_history IS 'Historial de transiciones de estado de pagos para conciliación y gestión de disputas.';

-- ============================================================
-- FIX v2 #12: TRIGGER updated_at AUTOMÁTICO EN TODAS LAS TABLAS
-- Bloque DO detecta automáticamente todas las tablas con updated_at
-- ============================================================
...
-- ============================================================
-- COMMENTS ADICIONALES (FIX-V3-17)
-- ============================================================

COMMENT ON TABLE  miles_transaction                         IS 'Transacciones de millas. ticket_id obligatorio para tipo EARN (validado por trigger). NULL permitido solo para ADJUST.';
COMMENT ON TABLE  seat_assignment                          IS 'Asignación de asiento por ticket_segment. Trigger trg_seat_assignment_aircraft_check garantiza consistencia con el avión del segmento.';
COMMENT ON TABLE  exchange_rate                            IS 'Tasas de cambio con tipo y fuente para billing multi-moneda auditado.';
COMMENT ON COLUMN address.is_active                        IS 'Soft-delete: false cuando la dirección es reemplazada, preservando referencias históricas en airport, maintenance_provider y airline.';
COMMENT ON COLUMN fare.is_current                          IS 'Columna generada: true si la tarifa está vigente hoy. Úsala con el índice parcial idx_fare_current.';
COMMENT ON COLUMN boarding_pass.expires_at                 IS 'Vencimiento del boarding pass. Normalmente = scheduled_departure_at del segmento.';
COMMENT ON COLUMN maintenance_provider.contact_person_id   IS 'FK a person para el contacto principal. Reemplaza contact_name varchar libre de v2.';
...
CREATE INDEX idx_address_is_active               ON address(is_active);

-- FIX-V3-16: índice de búsqueda por nombre de pasajero
...
CREATE INDEX idx_maintenance_event_status_id     ON maintenance_event(maintenance_event_status_id);
...
-- Índice parcial para consultas de tarifas vigentes
CREATE INDEX idx_fare_current                    ON fare(airline_id, origin_airport_id, destination_airport_id)
    WHERE is_current = true;
...
CREATE INDEX idx_invoice_line_source_entity_id   ON invoice_line(source_entity_id)
    WHERE source_entity_id IS NOT NULL;

-- Índices para historial de estados
CREATE INDEX idx_reservation_status_hist_res     ON reservation_status_history(reservation_id);
CREATE INDEX idx_reservation_status_hist_at      ON reservation_status_history(changed_at);
CREATE INDEX idx_ticket_status_hist_ticket       ON ticket_status_history(ticket_id);
CREATE INDEX idx_ticket_status_hist_at           ON ticket_status_history(changed_at);
CREATE INDEX idx_payment_status_hist_payment     ON payment_status_history(payment_id);
CREATE INDEX idx_payment_status_hist_at          ON payment_status_history(changed_at);
